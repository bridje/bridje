...
- and if you do faff about with =(def foo (let [f (fn [x y] ...)] foo))=, it's up to you to Deal With It™.
...
- so there's a spectrum of solutions here, from least to most permissive
  ...
  - in the middle is the 'top-level, tail position' hack
    - likewise, effects would only be on the GV
    - we'd only allow defaultable/overridden effects outside of this tail-fn
    - would disallow effects in things like =(-> (fn [...] ...) (wrap-... ...))=
      - we'd want this to behave the same within a function as we'd want at top-level - would be quite a gotcha to move a form and it not behave the same.
      - we legitimately can't optimise this, though - the handler fn's passed to the middleware, which calls it
      - we'd want this to be passed lexically
  - we've also got 'the function that gets returned from a top-level form'
    - which I think we have to put effects on the fntype too
  - is there a halfway house here whereby 'taking effects from the lexical environment' means dynamic for the whole of the top-level GV?
    - we could optimise it for the most common cases - turn it into one call when we know it's just a function in tail position
    - would mean that we wouldn't be able to have a top-level value (not fn) that was effectful but where the effects were only called once
      - maybe we could eval such effects at the start of a REPL call that we know requires it
    - so the practice is that 'whoever resolves the GV resolves the effects' - can that still apply here?
  - a different special form? i.e. =def!= for where we expect top-level side effects?
    - I do like the idea of users having to explicitly declare that they want compile-time effects
    - under this, =def= would implicitly be a 0-arg (1 with fx) function if it contained effects
    - no undefaultable effects in =def!=
    - we could optimise the tail-position case to be one call
- does this mean there's legitimately two different behaviours required by users in top-level definitions?
  - lexical for one-shot, top-level effects, dynamic for everything else?
  - even function calls - we may well want the middleware in the example above to use dynamic effects
- easier - we make the requirement that effectful defs have a fnexpr at the top-level - gives the user control over when the effects are resolved
  - how do top-level effects work in this case?
  - split into two - functions defined to be effects and functions calling effects
  - start with examples of legit top level effects - config, read at startup?
  - top level being 'compile time', essentially - which, if we're AoT compiling, I doubt we want?
  - alternatively, we don't ban it, we just say that dynamic effects only apply to ... the top-level tail function, again
    - we could resolve the GVs when they're included in the env, save them being 'compile-time' effects
    - still got the gotcha of extracting a function out, they might be within a with-fx scope where they were originally defined but move to global scope when they're extracted
      - maybe that means that these GVs should be automatically created as 0-arg functions if they're known to have effects
      - unless they're marked =def!=, which would run the effects at load time.
    - we could make it an /implicit/ zero-arg fn, if it doesn't have any arguments

the reason we're doing this is to prevent effects escaping a =with-fx= block, I think? otherwise we'd go full dynamic
- how often does this happen?
  - usages of with-fx
    - we're assuming them to be either at the top-level, or to satisfy 'reader'-like patterns
    - reader-like patterns could be quite local, in theory - just to satisfy a dependency of a block of code

how many calling patterns do we have?
- callee
  - effect fn
  - default impl
  - effectful fn
  - fn with closed over variables
  - effectful fn with closed over variables
    - think this is the same as the previous case, but with one extra closed over LV
  - with-fx fn
    - likewise
- caller
  - call expr with GV in first position
  - effectful GV outside of a call-expr first position
  - top-level call (something has to make the effect map?)

- effect fn and effectful fn look the same, to a caller
- closed over fn has a statically known set of closed over variables
  - in a closed over fn, we have a MF passed as the first argument, say?
  - we iterate over the FrameSlots of the MF (statically known) to re-set the locals
  - outer fn calling the inner fn keeps the MF
- we can inline the default impl into the effectvar's value
  - just need to keep track of the fact that there /is/ a default impl
- what's the data type?
- so do we have a 'closure' object separate to the others?
- or does BridjeFunction just have 'closed over variables', possibly empty?
