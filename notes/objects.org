...
#+begin_src clojure
  ...
  (:: :ConstantlyCountable Int)
  (def (. :ConstantlyCountable) (count (:ConstantlyCountable x))
    x)

  (:: (returns-something-countable) :ConstantlyCountable)
  (def (returns-something-countable)
    (:ConstantlyCountable 42))

  ;; need to define the order of type vars in typeclass functions, everywhere else, we don't need to?
