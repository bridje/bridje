...
```bridje
defkeys: {:term Int, :index Int, :command Str}
...
The `defkeys:`/`type:` separation is a Clojure-ism that pays for itself when keys are reused — `:from`, `:to`, `:term` appear in four message types and are declared once.
The batch `defkeys:` form with `:` sigil on keywords keeps this compact — for a single type in isolation it's comparable ceremony to Allium's inline declarations.
...
def: startElection(state, server)
  state.set!(:currentTerm, state.currentTerm.inc())
  state.set!(:role, Candidate({:votesReceived #{state.id}}))
  state.set!(:votedFor, state.id)
...
The state transitions read clearly — `set!(:currentTerm, ...)`, `set!(:role, ...)` are self-explanatory.
    ...
    cond:
      ...
      not(logConsistent(state, req))
        do:
          ...
          state.set!(:role, Follower({:knownLeader req.from}))
          server.onTimeout(clock.electionTimeout(), startElection, server)
          reject()
      ...
      do:
        ...
        state.set!(:role, Follower({:knownLeader req.from}))
        server.onTimeout(clock.electionTimeout(), startElection, server)
        state.set!(:log, state.log.take(req.prevLogIndex).append(req.entries))

        when: req.leaderCommit.gt(state.commitIndex)
          state.set!(:commitIndex, min(req.leaderCommit, state.log.count))
...
The `stepUpTerm` + `set!(:role, ...)` + `onTimeout` sequence is duplicated across the two non-stale branches.
...
State transitions (`set!(:currentTerm, ...)`, `set!(:role, ...)`) are self-documenting.
...
`state.set!(:commitIndex, newCommit)` vs `server.commit_index = new_commit`.
The `set!` form is more concise than the old per-field setter convention, and the `!` suffix signals mutation clearly.
