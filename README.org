...
Polymorphic keys are declared using the =.= special form. For example, the core
library declares a polymorphic =:Ok= variant which can contain a value of any
type:
...
=(:Ok 42)= (which has type =(+ (. Int :Ok))=) and eliminating it with =case=:
...
When both the type parameter and the key are simple symbols, we can shorten
=(:: (. a :Ok) a)= to =(:: :a.Ok a)=.

Polymorphic functions, likewise, are declared using the =.= special form
(although again, most of the time, these can use the short-form)
...
#+BEGIN_SRC clojure
  (:: (. a (count a)) Int) ; long form
  (:: (a.count a) Int) ; short form

  (:: :a.Cons a a.List)
  (:: :Nil)
  (:: a.List (+ :a.Cons :Nil))

  (def (. a.List (count list))
    (case list
