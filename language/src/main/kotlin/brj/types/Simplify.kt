...
private class Simplifier(scheme: Scheme, private val ctx: TypeCtx) {
    ...
    private fun resolve(v: TypeVar): Type {
        ...
        val result: Type = when {
            pos && !neg -> {
                ...
                parts.removeIf { it is BottomType }
                // (b ∧ F) ∨ b = b
                ...
                when {
                    parts.isEmpty() -> BottomType
                    parts.size == 1 -> parts.single()
                    // A variable or nil: the variable, nullable.
                    vars.size == 1 && parts.size == 2 && NilType in parts -> vars.single().nullable()
                    vars.isNotEmpty() -> rep
                    else -> {
    ...
    private fun nullableVars(t: Type, out: MutableSet<TypeVar>) {
        when (t) {
            ...
            is TypeVar, is PrimType, is RecordType, BottomType, NilType -> {}
        }
    }
    ...
    private fun subst(t: Type, pos: Boolean?, depth: Int = 0): Type {
        ...
        val r: Type = when (t) {
            ...
            is PrimType, is RecordType, BottomType, NilType -> t
        }
        return normalise(r)
    }
    ...
    private fun normalise(t: Type): Type = when (t) {
        ...
        is TypeVar, is PrimType, is RecordType, BottomType, NilType -> t
    }
