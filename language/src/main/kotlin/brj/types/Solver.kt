...
internal fun meetOf(base: Type, filter: Filter, ctx: TypeCtx): Type {
    ...
    return when (base) {
        ...
        NilType -> if (filter.record) fail("nil is not a record") else BottomType
...
private fun isNullish(t: Type) = t is NilType || t is NullableType
...
private class Solve(start: BoundEnv, private val ctx: TypeCtx) {
    ...
    fun constrain(lower: Type, upper: Type) {
        ...
        when {
            ...
            lower is NilType -> fail("nil is not a $upper, which is not nullable")
    ...
    private fun constrainMeet(lower: Meet, upper: Type) {
        ...
        when {
            ...
            upper is BottomType -> constrain(base, if (f.notNil) NilType else BottomType)
    ...
    private fun addUpper(tv: TypeVar, t: Type) {
        val b = upperOf(tv)
        when (t) {
            NilType -> setUpper(tv, b.copy(concrete = BottomType))
            BottomType -> setUpper(tv, b.copy(concrete = BottomType, nilOk = false))
    ...
    private fun meet(a: Type, b: Type): Type = when {
        a == b -> a
        a is BottomType || b is BottomType -> BottomType
        a is FnType && b is FnType -> {
            if (a.paramTypes.size != b.paramTypes.size) BottomType
            else FnType(
        ...
        a is RecordType && b is RecordType -> RecordType(a.keys + b.keys)
            ...
            val (bb, fb) = split(b)
            when {
                ctx.isNominal(ba) && ctx.isNominal(bb) -> nominalMeet(ba, bb)?.let { meetOf(it, fa and fb, ctx) } ?: BottomType
