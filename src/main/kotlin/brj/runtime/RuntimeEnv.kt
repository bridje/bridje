...
internal data class DefMacroVar(private val truffleEnv: TruffleLanguage.Env, override val sym: QSymbol, override val type: Type, val formsNS: NSEnv, override var value: Any?) : GlobalVar() {
    private fun fromVariant(obj: VariantObject): Form {
        ...
        return when (obj.variantKey.sym.base.baseStr) {
            "BooleanForm" -> BooleanForm(arg as Boolean, null)
            "StringForm" -> StringForm(arg as String, null)
            "IntForm" -> IntForm(arg as Long, null)
            "FloatForm" -> FloatForm(arg as Double, null)
            "BigIntForm" -> BigIntForm(arg as BigInteger, null)
            "BigFloatForm" -> BigFloatForm(arg as BigDecimal, null)
            "ListForm" -> ListForm(fromVariantList(arg), null)
            "VectorForm" -> VectorForm(fromVariantList(arg), null)
            "SetForm" -> SetForm(fromVariantList(arg), null)
            "RecordForm" -> RecordForm(fromVariantList(arg), null)
            "SymbolForm" -> SymbolForm(arg as Symbol, null)
            "QSymbolForm" -> QSymbolForm(arg as QSymbol, null)
            "QuotedSymbolForm" -> QuotedSymbolForm(arg as Symbol, null)
            "QuotedQSymbolForm" -> QuotedQSymbolForm(arg as QSymbol, null)
            else -> TODO()
        }
    }
