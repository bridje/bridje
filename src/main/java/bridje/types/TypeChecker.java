    ...
    static TypeResult typeExpr0(Expr<? extends Void> expr, LocalTypeEnv localEnv) {
        return expr.accept(new ExprVisitor<Void, TypeResult>() {
            ...
            public TypeResult visit(Expr.NSExpr<? extends Void> expr) {
                return new TypeResult(new Expr.NSExpr<>(expr.range, ENV_IO, expr.ns, expr.aliases, expr.refers, expr.imports));
            }
            ...
            @Override
            public TypeResult visit(Expr.DefJExpr<? extends Void> expr) {
                // TODO at some point we'll have to check the types that the user provides us with.
                // today is not that day, though.
