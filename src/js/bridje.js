...
cmd.command('build <entry-namespaces...>')
  ...
  .option('-p, --path <dir>[:<dir>...]', 'sets the search path for Bridje namespaces', path => path.split(':'))
...
cmd.command('run <main-ns> [args...]')
  .option('-p, --path <dir>[:<dir>...]', "sets the search paths for Bridje namespaces - separated by ':'", path => path.split(':'))
